...
| `radio_firmware/` | 34 MB | 21 | MCU firmware + RE artifacts: `DMR003.UV4T.V022-ORIGINAL.bin` (378,620 B, MD5 `4426…268E0`), `-PATCH14.bin`, `cmd_handler.c`, `firmware_decompiled.c`, `firmware_disassembly.txt` (32 MB), `ghidra_scripts/`, `ghidra_decompiled/project/`. | **Reference (RE).** ARM Thumb, uC/OS-III (confirmed), base `0x08000000` **assumed / unverified** — see deep-dive 07 §4.2, 18. |
...
| **Firmware patching (group-call fix)** | 14 patches at 5 contactType/CBZ/BLS locations, 0/14. Root cause (2026-08-27): patched mis-decoded bytes under an unverified base address (deep-dive 07 §4.2, 18 §4). Ghidra full decompile did not yield a working patch. | `FIRMWARE_PATCH_RESULTS.md`, `PATCH9/12/14_*`, `DECOMPILATION_STATUS.md` |
